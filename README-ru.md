...
- `$case` - одна из констант `morphos\Russian\Cases` ИЛИ строка (`родительный`, `дательный`, `винительный`, `творительный` или `предложный`).
- `$gender` - одна из констант `Gender` (`morphos\Gender::MALE` или `morphos\Gender::FEMALE`) ИЛИ строка (`m` для мужского имени,  `"f"` для женского имени) ИЛИ `null` для автоматического определения.
...
Аргументы:
- `$name` - полное имя в формате `Имя`, `Фамилия Имя` или `Фамилия Имя Отчество`.
- `$gender` - одна из констант `morphos\Gender` ИЛИ строка (`"m"` для мужского имени,  `"f"` для женского имени), `null` для автоматического определения.
