...
- `$case` - одна из констант `Cases` ИЛИ строка (`родительный`, `дательный`, `винительный`, `творительный` или `предложный`).
- `$gender` - одна из констант `Gender` ИЛИ строка (`"m"` для мужского имени,  `"f"` для женского имени), `null` для автоматического определения.
